...
UPDATE POPULATION_EXPOSURE SET EXPOSEDHOSPITALS = COALESCE((SELECT COUNT(*) FROM BUILDINGS_MAX_ERPS R  WHERE r.NOISELEVEL = POPULATION_EXPOSURE.NOISELEVEL AND r.AGGLO = (exposureType = 'mostExposedFacadeIncludingAgglomeration') AND ERPS_NATUR='Sante'), EXPOSEDHOSPITALS),
  		EXPOSEDSCHOOLS   = COALESCE((SELECT COUNT(*) FROM BUILDINGS_MAX_ERPS R  WHERE r.NOISELEVEL = POPULATION_EXPOSURE.NOISELEVEL AND r.AGGLO = (exposureType = 'mostExposedFacadeIncludingAgglomeration') AND ERPS_NATUR='Enseignement'), EXPOSEDSCHOOLS)
  WHERE UUEID = @UUEID;
...
UPDATE POPULATION_EXPOSURE B SET EXPOSEDAREA = COALESCE((SELECT sum(P.EXPOSEDAREA) FROM POPULATION_EXPOSURE P,
 noise_level_class C where P.NOISELEVEL = C.NOISELEVEL and C.EXPOSURENOISELEVEL=B.NOISELEVEL
 and EXPOSURETYPE  = B.EXPOSURETYPE and UUEID = @UUEID), EXPOSEDAREA) WHERE UUEID = @UUEID;
