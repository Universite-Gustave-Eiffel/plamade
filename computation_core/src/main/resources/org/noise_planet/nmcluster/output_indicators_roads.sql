...
create table receiver_expo as SELECT PK_1, (CASE WHEN LAEQ < 55 THEN NULL WHEN LAEQ < 60 THEN 'Lden5559' WHEN LAEQ < 65 THEN 'Lden6064' WHEN LAEQ < 70 THEN 'Lden6569' WHEN LAEQ < 75 THEN 'Lden7074' ELSE 'LdenGreaterThan75' END) NOISELEVEL, BUILD_PK, PERCENT_RANK() OVER (PARTITION BY BUILD_PK ORDER BY LAEQ DESC, PK_1) RECEIVER_RANK  FROM LDEN_ROADS L, RECEIVERS_UUEID RU, RECEIVERS_BUILDING RB WHERE RCV_TYPE = 1 AND L.IDRECEIVER = RU.PK AND PK_1 = RB.PK order by BUILD_PK, LAEQ DESC;
-- remove receivers with noise level inferior than median noise level for the same building
DELETE FROM receiver_expo WHERE RECEIVER_RANK > 0.5 OR NOISELEVEL IS NULL;
