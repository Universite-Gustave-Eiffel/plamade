...
INSERT INTO BUILDINGS_MAX SELECT @UUEID UUEID, ID_BAT, AGGLO, 'LDEN' PERIOD,max(LAEQ) - 3 noiselevel FROM BUILDINGS_SCREENS B INNER JOIN RECEIVERS_BUILDING RB ON (B.PK = RB.BUILD_PK) INNER JOIN RECEIVERS_UUEID  RU ON (RB.PK = RU.PK_1 AND RU.RCV_TYPE = 1) INNER JOIN LDEN_ROADS LR ON (RU.PK = LR.IDRECEIVER) GROUP BY ID_BAT, AGGLO;
INSERT INTO BUILDINGS_MAX SELECT @UUEID UUEID, ID_BAT, AGGLO, 'LN' PERIOD,max(LAEQ) - 3 noiselevel FROM BUILDINGS_SCREENS B INNER JOIN RECEIVERS_BUILDING RB ON (B.PK = RB.BUILD_PK) INNER JOIN RECEIVERS_UUEID  RU ON (RB.PK = RU.PK_1 AND RU.RCV_TYPE = 1) INNER JOIN LNIGHT_ROADS LR ON (RU.PK = LR.IDRECEIVER) GROUP BY ID_BAT, AGGLO;
