...
UPDATE POPULATION_EXPOSURE SET
 ...
 AND r.LAEQ < POPULATION_EXPOSURE.MAX_LAEQ  AND r.PERIOD = 'LDEN'  AND (NOT r.AGGLO OR exposureType = 'mostExposedFacadeIncludingAgglomeration')
 ...
 AND r.LAEQ < POPULATION_EXPOSURE.MAX_LAEQ  AND r.PERIOD = 'LDEN'  AND (NOT r.AGGLO OR exposureType = 'mostExposedFacadeIncludingAgglomeration')
   AND ERPS_NATUR='Enseignement'), EXPOSEDSCHOOLS) WHERE UUEID = @UUEID AND POPULATION_EXPOSURE.NOISELEVEL LIKE 'Lden%';

-- UPDATE NIGHT
 UPDATE POPULATION_EXPOSURE SET
    EXPOSEDHOSPITALS = COALESCE((SELECT COUNT(*) FROM BUILDINGS_MAX_ERPS R  WHERE r.LAEQ >= POPULATION_EXPOSURE.MIN_LAEQ
    AND r.LAEQ < POPULATION_EXPOSURE.MAX_LAEQ AND r.PERIOD = 'LN' AND  (NOT r.AGGLO OR exposureType = 'mostExposedFacadeIncludingAgglomeration')
    AND ERPS_NATUR='Sante'), EXPOSEDHOSPITALS),
     EXPOSEDSCHOOLS   = COALESCE((SELECT COUNT(*) FROM BUILDINGS_MAX_ERPS R  WHERE
     r.LAEQ >= POPULATION_EXPOSURE.MIN_LAEQ
    AND r.LAEQ < POPULATION_EXPOSURE.MAX_LAEQ AND r.PERIOD = 'LN' AND (NOT r.AGGLO OR exposureType = 'mostExposedFacadeIncludingAgglomeration')
      AND ERPS_NATUR='Enseignement'), EXPOSEDSCHOOLS) WHERE UUEID = @UUEID  AND POPULATION_EXPOSURE.NOISELEVEL LIKE 'Lnight%';
