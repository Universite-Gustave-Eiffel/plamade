...
def exec(Connection connection, input) {
   ...
   // for(int idep = 1;idep<100;idep++) {
     for(int idep = 971;idep<990;idep++) {   
         println(idep);
    ...
    String codeDep = idep//input["inseeDepartment"] as String
   ...
   // -------------------
    ...
    sql2.execute("DROP TABLE IF EXISTS LW_ROADS;")
    sql2.execute("CREATE TABLE LW_ROADS (pk integer, the_geom Geometry, ID_TRONCON varchar, UUEID varchar, LWDEN double precision," +
            "LWD double precision,LWD63 double precision, LWD125 double precision, LWD250 double precision, LWD500 double precision, LWD1000 double precision, LWD2000 double precision, LWD4000 double precision, LWD8000 double precision," +
            "LWE double precision,LWE63 double precision, LWE125 double precision, LWE250 double precision, LWE500 double precision, LWE1000 double precision, LWE2000 double precision, LWE4000 double precision, LWE8000 double precision," +
            "LWN double precision,LWN63 double precision, LWN125 double precision, LWN250 double precision, LWN500 double precision, LWN1000 double precision, LWN2000 double precision, LWN4000 double precision, LWN8000 double precision);")

    def qry = 'INSERT INTO LW_ROADS(pk,the_geom, ID_TRONCON, UUEID,LWDEN,' +
            'LWD, LWD63, LWD125, LWD250, LWD500, LWD1000,LWD2000, LWD4000, LWD8000,' +
            'LWE,LWE63, LWE125, LWE250, LWE500, LWE1000,LWE2000, LWE4000, LWE8000,' +
            'LWN,LWN63, LWN125, LWN250, LWN500, LWN1000,LWN2000, LWN4000, LWN8000) ' +
            'VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);'
   ...
   PreparedStatement st = connection.prepareStatement("SELECT COUNT(*) AS total FROM ROADS"  )
    ...
    sql2.withBatch(100, qry) { ps ->
        PreparedStatement st2 = connection.prepareStatement("SELECT * FROM ROADS")
        SpatialResultSet rs = st2.executeQuery().unwrap(SpatialResultSet.class)
        ...
        while (rs.next()) {
            ...
            double lday_ = ComputeRays.wToDba(ComputeRays.sumArray(ComputeRays.dbaToW(ComputeRays.sumArray(ComputeRays.wToDba(results[0]),dba))))
            double levening_ = ComputeRays.wToDba(ComputeRays.sumArray(ComputeRays.dbaToW(ComputeRays.sumArray(ComputeRays.wToDba(results[1]),dba))))
            double lnight_ = ComputeRays.wToDba(ComputeRays.sumArray(ComputeRays.dbaToW(ComputeRays.sumArray(ComputeRays.wToDba(results[2]),dba))))
           
            double lden_ = ComputeRays.wToDba((12*ComputeRays.dbaToW(lday_) + 4*ComputeRays.dbaToW(levening_+5) + 8*ComputeRays.dbaToW(lnight_+10))/24)
    ...
    sql2.execute("UPDATE LW_ROADS SET THE_GEOM = ST_UPDATEZ(The_geom,0.05);")

    // Add primary key to the road table
    sql2.execute("ALTER TABLE LW_ROADS ALTER COLUMN PK INT NOT NULL;")
    sql2.execute("ALTER TABLE LW_ROADS ADD PRIMARY KEY (PK);")    
    sql2.execute("UPDATE LW_ROADS SET THE_GEOM = ST_SETSRID(THE_GEOM, "+sridSources+")")

    sql2.execute("CREATE SPATIAL INDEX ON LW_ROADS(THE_GEOM);")

 
    sql2.execute("CALL SHPWrite(CONCAT('C:/Users/aumond/Documents/Projets/2020_XXX Plamade/LW/LW_', CONCAT( $codeDep , '.shp')), 'LW_ROADS');")
}
