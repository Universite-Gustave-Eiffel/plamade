...
def exec(Connection connection, input) {
    ...
    if (JDBCUtilities.tableExists(connection, rail_sections)){
        !tableExists
    }

    if (tableExists) {
        int nrows = JDBCUtilities.getRowCount(connection.connection, "rail_sections")
        println("Number of rows in the table: " + nrows)
    } else {
        println("The table ${rail_sections} does not exist.")
    }
    ...
    sql.execute("CREATE TABLE LW_ROADS (pk integer, the_geom Geometry, ID_TRONCON varchar, " +
    ...
    def qry = 'INSERT INTO LW_ROADS(pk,the_geom, ID_TRONCON, ' +
    ...
    sql.withBatch(100, qry) { ps ->
        ...
        while (rs.next()) {
            ...
            ps.addBatch(rs.getLong(pkIndex) as Integer, geo as Geometry, rs.getString(2) as String,
