...
def exec(Connection connection, input) {
    ...
    def resolution = 25 // When the dem is merged with road and rail sections, the resolution of the created point is 25m
    ...
    def queries_dem_rail = """
    ALTER TABLE RAIL_SECTIONS ADD pk_line INT AUTO_INCREMENT;
    ...
    DELETE FROM DEM_WITHOUT_PTLINE WHERE EXISTS (SELECT 1 FROM RAIL_SECTIONS  b WHERE ST_EXPAND(DEM_WITHOUT_PTLINE.THE_GEOM,''$maxWidth'' , ''$maxWidth'' )   && b.the_geom AND ST_DISTANCE(DEM_WITHOUT_PTLINE .THE_GEOM, b.the_geom )< b.width LIMIT 1) ;

    DROP TABLE BUFFERED_PTLINE IF EXISTS;
    CREATE TABLE BUFFERED_PTLINE AS SELECT st_tomultipoint(st_densify(st_buffer(the_geom, WIDTH ,'endcap=flat'), ''$resolution'' )) the_geom from RAIL_SECTIONS ;

    DROP TABLE IF EXISTS BUFFERED_PTLINE_EX;
    create table BUFFERED_PTLINE_EX as select * from st_explode('BUFFERED_PTLINE');
    DROP TABLE BUFFERED_PTLINE IF EXISTS;
    CREATE SPATIAL INDEX ON BUFFERED_PTLINE_EX(THE_GEOM);

    DROP TABLE IF EXISTS BUFFERED_PTLINE_Z ;
    CREATE TABLE BUFFERED_PTLINE_Z  AS SELECT ST_ADDZ(ST_Force3D(r.THE_GEOM),  ST_Z(SELECT ST_ProjectPoint( r.THE_GEOM,t.THE_GEOM) THE_GEOM FROM RAIL_SECTIONS t where st_expand(r.the_geom,''$maxWidth'', ''$maxWidth'')  && t.the_geom order by st_distance(r.the_geom, t.the_geom) LIMIT 1)) the_geom FROM BUFFERED_PTLINE_EX r ; 
    ...
    logger.info('Merge dem with roads (11/13)')
    engine = new SimpleTemplateEngine()
    template = engine.createTemplate(queries_dem_roads).make(binding)
    sql.execute(template.toString())

    /*logger.info('Merge dem with rails (12/13)')
    engine = new SimpleTemplateEngine()
    template = engine.createTemplate(queries_dem_rail).make(binding)
    sql.execute(template.toString())

    logger.info('Manage statistics (13/13)')
    engine = new SimpleTemplateEngine()
