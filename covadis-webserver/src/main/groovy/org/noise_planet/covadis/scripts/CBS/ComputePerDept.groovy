...
title = 'Full NoiseModelling computation that computes isosurfaces for all UUEID of a department at the same time'
description = 'Full NoiseModelling computation that computes isosurfaces for all UUEID of a department at the same time, the receiver sound level is the sum of the sound level contributions of all the UUEID'
...
inputs = [
        ...
        department    : [
                ...
                description: "Department code as found in the UUEID (3 characters) eg. 078, 044, 973",
                type       : String.class
        ],
        conf          : [
...
def exec(Connection connection, Map input, ProgressVisitor progress) {
    ...
    try (DataSource dataSource = PostGISUtilities.createPostgisDataSource(
        ...
        String department = input.department as String
        String uueidPattern = "RD_FR_00_${department}%"
        logger.info("Computing all UUEID matching pattern: $uueidPattern")

        List<String> uueids = new ArrayList<>()
        sql.rows("SELECT DISTINCT uueid from cbs_uge_output.routier_emission_${input.projectionName} WHERE uueid LIKE '$uueidPattern'" as String).each {
            row ->
                uueids.add(row.uueid as String)
        }

        if (uueids.isEmpty()) {
            throw new IllegalArgumentException("No match for the provided department '${input.department}'")
        }
        Set<String> matchedDepartments = uueids.collect { it.split("_")[3].substring(0, 3) }.toSet()
        if (matchedDepartments.size() > 1 || !matchedDepartments.contains(department)) {
            throw new IllegalArgumentException("The provided department '${input.department}' does not match the department(s) of the matched UUEIDs ${matchedDepartments}")
        }

        ProgressVisitor stepsProgress = progress.subProcess(1) // long running sub tasks

        def tempDirectory = File.createTempDir()
        // Create a single local H2 database for the whole department
        DataSource h2DataSource = DatabaseManagement.createH2DataSource(tempDirectory.getAbsolutePath() ,
                "h2_${department}", "sa", "sa", "", true)
        logger.info("Create database for department: $department in directory: $tempDirectory")
        ...
        computeForDepartment(uueidPattern, department, h2DataSource, pgConnection, stepsProgress, input, mainConfiguration, codeDeptToNuts)

        // Delete the database file
        if(h2DataSource instanceof Closeable) {
            ((Closeable) h2DataSource).close()
        }
        new File(tempDirectory, "h2_${department}.mv.db").delete()
...
def computeForDepartment(String uueidPattern, String department, DataSource h2DataSource, Connection pgConnection, ProgressVisitor progress, Map input, Map mainConfiguration, Map<String, String> codeDeptToNuts) {
    ProgressVisitor stepsProgress = progress.subProcess(3) // long running sub tasks

    def pgSql = new Sql(pgConnection)
    Logger logger = LoggerFactory.getLogger(this.class)
    logger.info("Computing for department: $department, uueid pattern: $uueidPattern")
    try(Connection h2Connection = h2DataSource.getConnection()) {
        ...
        def res = pgSql.firstRow("""SELECT 
            ...
            mainConfiguration.confmaxsrcdist * 1.2 + mainConfiguration.confmaxrefldist}), 25) geomenv
             FROM cbs_uge_output.routier_emission_${input.projectionName} AS reg WHERE uueid LIKE '$uueidPattern';""" as String)
        if (res == null) {
            throw new IllegalArgumentException("No match for the provided department '${input.department}'")
        }
        ...
        fetchDem(input, uueidPattern, extractionEnvelopeGeometryWKT, h2Connection, pgConnection, stepsProgress)
        ...
        generateReceivers(input, pgConnection, uueidPattern, extractionEnvelopeGeometry, h2Connection,
        ...
        fetchAtmosphericPeriodFromStations(input, pgConnection, uueidPattern, h2Connection, stepsProgress)
    }
    ...
    new ArrayList<>(posSols).forEach {posSol ->
        ...
        boolean doCompute
        try(Connection h2Connection = h2DataSource.getConnection()) {
            doCompute = fetchRoads(input, pgConnection, uueidPattern, h2Connection, solProgress, posSol, mainConfiguration.confmaxsrcdist * 1.2d as Double)
        }
        // Fetch specific road emission at this special height
        if(doCompute) {
            try(Connection h2Connection = h2DataSource.getConnection()) {
                // Adapt the DEM with this special road height platforms
                enrichDem(input, uueidPattern, h2Connection, pgConnection, solProgress, posSol)
            }
    ...
    try(Connection h2Connection = h2DataSource.getConnection()) {
        ...
        mergeReceiversLevels(posSols, h2Connection, department, logger, h2Sql)
...
static def generateExposureStatisticsFromFacadeExpo(Connection h2Connection, String uueid, Map<String, String> codeDeptToNuts, String projectionName) {
    ...
    Logger logger = LoggerFactory.getLogger(this.class)
    // 1. Extract metadata, uueid contains the department code
    def codeDept = uueid
 ...
 * @param uueidPattern UUEID pattern used to extract the roads
 ...
 * @return
 */
def generateReceivers(Map input,Connection pgConnection,String uueidPattern, Geometry extractionEnvelopeGeometry, Connection h2Connection,double deltaBuildingsReceivers, Map mainConfiguration, ProgressVisitor stepsProgress) {
    Logger logger = LoggerFactory.getLogger(this.class)
...
def fetchDem(Map input, String uueidPattern, String extractionEnvelopeGeometry, Connection h2Connection, Connection pgConnection, ProgressVisitor stepsProgress) {
    Sql pgSql = new Sql(pgConnection)
    Logger logger = LoggerFactory.getLogger(this.class)
    ...
    def fetchTableNamesQuery = """
        ...
        WHERE nldirh.uueid LIKE '$uueidPattern';
    """
...
def enrichDem(Map input, String uueidPattern, Connection h2Connection, Connection pgConnection, ProgressVisitor stepsProgress, String posSol) {
    Logger logger = LoggerFactory.getLogger(this.class)
    logger.info("Adapting digital elevation model..")
    ProgressVisitor demProgress = stepsProgress.subProcess(2)

    // Fetch road table with altitude using the UUEID pattern
    def roadQuery = """SELECT geom as the_geom, largeur as width
        FROM cbs_uge_input.n_routier_troncon_l_${input.projectionName}
        WHERE uueid LIKE '${uueidPattern}' and pos_sol = '$posSol' and (franchisst is null or franchisst = 'Pont')"""
    try( Statement st = pgConnection.createStatement() ;
         ResultSet rs = st.executeQuery(roadQuery)) {
        PostGISUtilities.copyResultSetToDatabase(pgConnection, rs, h2Connection, "ROADS", true, batchSize)
        demProgress.endStep()
...
/**
 * Fetches atmospheric periods from nearby stations for a given UUEID pattern.
 * @param input The input map containing configuration parameters.
 * @param uueidPattern The road UUEID pattern for which to fetch atmospheric periods.
 * @param h2Connection The H2 database connection.
 * @param stepsProgress The progress visitor for tracking execution progress.
 */
def fetchAtmosphericPeriodFromStations(Map input,Connection pgConnection, String uueidPattern, Connection h2Connection, ProgressVisitor stepsProgress) {
    ...
    Logger logger = LoggerFactory.getLogger(this.class)
    logger.info("Fetch nearest atmospheric points periods for UUEID: {}", uueidPattern)
    ...
    logger.info("Distance of ${uueidPattern} to nearest station: ${Math.round(distanceStation.distance_station as double)} m")
...
def fetchRoads(Map input, Connection pgConnection, String uueidPattern, Connection h2Connection, ProgressVisitor stepsProgress, String posSol, double maxSourceDistance) {
    Logger logger = LoggerFactory.getLogger(this.class)
    logger.info("Fetch roads..")
    Sql h2Sql = new Sql(h2Connection)
    def roadsQuery = """SELECT * FROM cbs_uge_output.routier_emission_${input.projectionName} WHERE uueid LIKE '$uueidPattern' AND pos_sol='$posSol' AND (franchisst IS NULL OR franchisst = 'Pont')"""
